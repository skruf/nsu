...
const actions = {
  async listAsync({ commit, state }, filter = {}) {
    ...
    const results = await findClubsAsync(filter, options)
    commit("SET_LIST", results.clubs)
    commit("SET_COUNT", results.count)
    commit("SET_LIST_LOADING", false)
  },
  async selectAsync({ commit }, filter = {}) {
    commit("SET_SELECTED_LOADING", true)
    const item = await findOneClubAsync(filter)
